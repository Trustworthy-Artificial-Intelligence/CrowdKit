...
num = 10
T_list = np.random.randint(0, 12, num)*0.1
gamma_list_list = []
for j in range(6):
    gamma_list_list.append([np.random.uniform(j+1, j+2) for i in range(num)])
...
plt.bar(range(len(a)), r_list, width=bar_width, label='My')
plt.bar(x_2, r_max_list, width=bar_width, label='Max')
plt.bar(x_3, r_random_list, width=bar_width, label='Random')
...
bar_width = 0.2
x_4 = list(range(len(b)))
x_5 = [i+bar_width for i in x_4]
x_6 = [i+bar_width*2 for i in x_4]

plt.figure(figsize=(4, 4), dpi=150)
plt.bar(range(len(b)), client_utility_avg_gamma, width=bar_width, label='My')
plt.bar(x_5, client_utility_max_avg_gamma, width=bar_width, label='Max')
plt.bar(x_6, client_utility_random_avg_gamma, width=bar_width, label='Random')
