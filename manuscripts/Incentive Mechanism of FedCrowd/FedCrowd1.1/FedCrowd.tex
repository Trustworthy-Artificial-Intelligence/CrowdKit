...
\begin{document}
	...
	\begin{frontmatter}
		...
		\begin{abstract}
			Crowdsourcing is an effective application to solve difficult tasks that cannot be solved by computers alone through a combination of human intelligence. However, collecting data from massive Internet workers through crowdsourcing may expose them to the risk of privacy breaches because of the sharing of personal data, which will reduce workers’ willingness to participate in crowdsourcing tasks. To address this series of problems, we introduce federated learning as well as design an incentive mechanism to put forward TiFedCrowd, which aims to mitigate the risk of privacy breaches for crowd workers while obtaining high-quality crowdsourcing data at the most economical cost possible. TiFedCrowd allows clients to keep privacy-related data locally while only needing to upload the trained model parameters to the server. The server then aggregates these uploaded models to train the global model. TiFedCrowd also models the federated crowdsourcing process as a two-stage Stackelberg game to motivate more workers to complete crowdsourcing tasks with high quality and efficiency concurrently. Meanwhile, we also define a time control mechanism to manage the quality of the submitted model and the range of data freshness and waiting time for the data requester. TiFedCrowd requires the client to complete the federated crowdsourcing task within a specified time interval while maximizing the utility of both the clients and the server. We find the unique global optimal solution through the unique existence of Nash equilibrium. We also extended TiFedCrowd to the multiple heterogeneous federated crowdsourcing scenario. Finally, we conduct experiments to verify the effectiveness of TiFedCrowd.
		\end{abstract}
		\begin{keyword}
			crowdsourcing learning, federated learning (FL), privacy protection, incentive mechanism, Stackelberg game 
		\end{keyword}
	\end{frontmatter}
...
In this section, we conducted some experiments to describe and certify that the incentive mechanism of our TiFedCrow is effective. In subsection~\ref{PCWB}, we compared the performance of TiFedCrowd with several baselines to demonstrate the optimal applicability of Tifed. In subsection~\ref{RTTCM}, we removed the time control mechanism first. Then we compared the performance with TiFedCrow to illustrate the importance of time control mechanism. In subsection~\ref{ITNC}, we observed the impact of the number of clients on the performance of TiFedCrowd. In subsection~\ref{ERCP}, We verified the effectiveness of TiFedCrowd on a real dataset. In subsection~\ref{CTCT}, we compared the time taken by the server to calculate the reward rate in TiFedCrow and iFedCrow \citep{kang2022incentive} to show that TiFedCrow is superior to iFedCrow not only in the time control mechanism but also in the calculation time. Each experiment was run 20 times separately to calculate the average result. 

\subsection{Performance Comparison with Baselines} \label{PCWB}
...
\begin{itemize}
	\item MAX: To select the largest reward rate to obtain the best response. 
	\item MIN: To pick the smallest reward rate to pay the minimum incentive cost.
	\item RANDOM: To randomly choose the reward rate to motivate the clients. 
\end{itemize}
...
\subsection{Remove The Time Control Mechanism}\label{RTTCM}
...
\subsection{Experiment with Real Crowdsourcing Project} \label{ERCP}
...
\subsection{Comparison of The Calculation Time} \label{CTCT}
